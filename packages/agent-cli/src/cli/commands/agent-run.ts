      ...
      try {
        ...
        const callbacks: OrchestratorCallbacks = {
          ...
          onSubtaskStart: (subtask: SubTask, progress: Progress) => {
            if (!jsonOutput) {
              ctx.ui?.write(
                `│  ⏳ [${progress.current}/${progress.total}] ${subtask.agentId}...\n`,
              );
            }
          },
          ...
          onSubtaskComplete: (
            ...
            if (!jsonOutput) {
              ctx.ui?.write(
                `│  ✅ [${progress.current}/${progress.total}] ${subtask.agentId} (${result.durationMs}ms, ${result.tokensUsed} tokens)\n`,
              );
            }
          },
          ...
          onSubtaskFailed: (
            ...
            if (!jsonOutput) {
              ctx.ui?.write(
                `│  ❌ [${progress.current}/${progress.total}] ${subtask.agentId} - ${result.error || "unknown error"}\n`,
              );
            }
          },
          ...
          onAdaptation: (reason: string, newSubtasks: SubTask[]) => {
            if (!jsonOutput) {
              ctx.ui?.write(
                `│  ✨ Plan adapted: ${reason} (+${newSubtasks.length} subtasks)\n`,
              );
            }
          },
            ...
            if (!jsonOutput) {
              const successRate =
                stats.totalSubtasks > 0
                  ? Math.round(
                      (stats.successfulSubtasks / stats.totalSubtasks) * 100,
                    )
                  : 0;
              ctx.ui?.write(
                `│  🎉 Completed: ${stats.successfulSubtasks}/${stats.totalSubtasks} succeeded (${successRate}%)\n`,
              );
        ...
        if (jsonOutput) {
          ...
          for (const delegatedResult of result.delegatedResults) {
            const subtask = result.plan.find(
              (s) => s.id === delegatedResult.subtaskId,
            );
            const status = delegatedResult.success ? "✅" : "❌";

            console.log(
              `│  ${status} [${delegatedResult.subtaskId}] ${subtask?.description || "(unknown)"}`,
            );
          ...
          console.log(
            `│  Total Duration: ${result.durationMs}ms (${(result.durationMs / 1000).toFixed(1)}s)`,
          );
          const successRate =
            result.delegatedResults.length > 0
              ? (
                  (result.delegatedResults.filter((r) => r.success).length /
                    result.delegatedResults.length) *
                  100
                ).toFixed(0)
              : 0;
          ...
          if (result.success) {
            console.log(
              "└── Success / " + (result.durationMs / 1000).toFixed(1) + "s\n",
            );
          } else {
            console.log(
              "└── Failed / " + (result.durationMs / 1000).toFixed(1) + "s\n",
            );
            if (result.error) {
