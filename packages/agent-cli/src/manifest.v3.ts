...
export const manifest = defineManifest({
  ...
  display: {
    name: "KB Labs Agents",
    description:
      "Autonomous agents powered by LLMs with tool calling capabilities",
    ...
    commands: [
      {
        ...
        longDescription:
          "Creates the .kb/agents/ directory where agent configuration files (YAML) can be placed. This is the first step in setting up agents in your project.",
      ...
      },
      {
        ...
        longDescription:
          "Scans .kb/agents/ directory and displays all available agent configurations with their tools and capabilities.",
        flags: defineCommandFlags({
          json: {
            type: "boolean",
      ...
      },
      {
        ...
        longDescription:
          "Shows the agent configuration including system prompt, available tools (including submit_result), output schema, and execution limits. Useful for debugging agent behavior.",
        flags: defineCommandFlags({
          agentId: {
            type: "string",
        ...
        examples: generateExamples("inspect", "agent", [
          {
            description: "Inspect implementer agent",
            flags: { agentId: "implementer" },
          },
          {
            description: "Inspect researcher agent",
            flags: { agentId: "researcher" },
          },
          {
            description: "Inspect reviewer agent",
            flags: { agentId: "reviewer" },
          },
        ]),
      ...
      },
      {
        ...
        group: "agent",
        describe:
          "Execute a complex task via orchestrator with agent delegation",
        longDescription:
          "Runs an orchestrator that breaks complex tasks into subtasks, delegates them to agents, and synthesizes results into a coherent answer. Uses smart tier LLM for planning and synthesis.",
        flags: defineCommandFlags({
          task: {
            type: "string",
        ...
        examples: generateExamples("run", "agent", [
          {
            ...
            flags: {
              task: "Analyze the V2 agent architecture and create implementation plan",
            },
          },
          {
            description: "Research and implement feature",
            flags: {
              task: "Find how hybrid search works and add similar functionality to workflow engine",
            },
          },
          {
            description: "Output as JSON",
            flags: {
              task: "Explain the plugin system architecture",
              json: true,
            },
          },
        ]),
