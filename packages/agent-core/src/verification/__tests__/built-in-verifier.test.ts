...
import type { FileWriteClaim, FileEditClaim, FileDeleteClaim } from '@kb-labs/agent-contracts';
    ...
    it('should fail when no anchors found', async () => {
      ...
      const claim: FileEditClaim = {
        ...
        anchor: {
          beforeSnippet: 'function foo() {',
          afterSnippet: "console.log('hello');",
          contentHash: createHash('sha256').update('original content', 'utf-8').digest('hex'),
        },
      };
      ...
      const claims = [
        {
          ...
          filePath: 'file1.txt',
          contentHash: createHash('sha256').update(file1Content, 'utf-8').digest('hex'),
        },
        {
          kind: 'file-write' as const,
          filePath: 'file2.txt',
          contentHash: createHash('sha256').update(file2Content, 'utf-8').digest('hex'),
        },
      ];
      ...
      const claims = [
        {
          kind: 'file-write' as const,
          filePath: 'exists.txt',
          contentHash: createHash('sha256').update('test', 'utf-8').digest('hex'),
        },
        {
          kind: 'file-write' as const,
          filePath: 'nonexistent.txt',
