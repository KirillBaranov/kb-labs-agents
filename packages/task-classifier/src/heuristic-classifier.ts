...
import type { LLMTier, ClassifyInput, ClassificationResult, HeuristicRule, ITaskClassifier } from './types.js';
...
const DEFAULT_RULES: HeuristicRule[] = [
  ...
  {
    keywords: ['find', 'search', 'list', 'show', 'get', 'read', 'check', 'verify'],
    tier: 'small',
    weight: 0.8,
  },
  {
    keywords: ['найди', 'найти', 'покажи', 'показать', 'список', 'прочитай', 'проверь'],
    tier: 'small',
    weight: 0.8,
  },
  ...
  {
    keywords: ['implement', 'add', 'create', 'update', 'modify', 'fix', 'refactor', 'test'],
    tier: 'medium',
    weight: 0.7,
  },
  {
    keywords: ['реализуй', 'реализовать', 'добавь', 'добавить', 'создай', 'создать', 'исправь', 'исправить', 'обнови', 'обновить'],
    tier: 'medium',
    weight: 0.7,
  },
  ...
  {
    keywords: ['design', 'architect', 'plan', 'analyze', 'optimize', 'migrate', 'scale', 'integrate'],
    tier: 'large',
    weight: 0.9,
  },
  {
    keywords: ['спроектируй', 'спроектировать', 'проанализируй', 'проанализировать', 'оптимизируй', 'оптимизировать', 'интегрируй', 'интегрировать'],
    tier: 'large',
    weight: 0.9,
  },
...
export class HeuristicComplexityClassifier implements ITaskClassifier {
  ...
  async classify(input: ClassifyInput): Promise<ClassificationResult> {
    ...
    for (const rule of this.rules) {
      if (rule.keywords.length === 0) continue; // Skip length-based rule
    ...
    if (/\b(multi-step|complex|comprehensive|detailed|thorough)\b/i.test(taskDescription)) {
      scores.large += 0.5;
    }
    ...
    const confidence: 'high' | 'low' = score - secondHighest >= 0.5 ? 'high' : 'low';
